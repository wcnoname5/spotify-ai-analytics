...
- **Env B — full parallel stack (only to exercise Cloud/cron/seed).** Env A **plus** a separate test
  Worker + test D1. Cleanest isolation = a **second Cloudflare account** (run `setup_cloud.sh`
  as-is). Same-account alternative: distinct `name`/`database_name` in `worker/wrangler.toml` +
  `PROD_DB` in the script so it can't clobber prod.

### Env A runbook (PowerShell, from your own terminal)
...
2. **Step 1 Client ID** → paste the **second account's** ID, Save → advances to step 2. Confirm a
   "back up your encryption key" notice appears (currently missing — Finding D).
...
5. **Step 5 LLM** → try to **Skip without a key** — today you can't (Finding A). Note it.
...
### Env B addendum (only to test Cloud)

After Env A works: create the test Worker/D1 (options above), then in the scratch `.env` set
`WORKER_URL`/`WORKER_AUTH_TOKEN` to the **test** Worker, give that Worker its secrets (`AUTH_TOKEN`
new, `SPOTIFY_CLIENT_ID` = second account, `TOKEN_ENCRYPT_KEY` = the **scratch** `.env`'s key), and
run `seed_d1.py`. Verify the app pulls from the test D1 and the test cron ticks (`wrangler tail`).
Never reuse prod's Worker URL or D1 name.
...
**§3 Cloud deploy (`scripts/setup_cloud.sh`).** .env (client_id + gen Fernet key) → `wrangler login`
→ `d1 create` → patch `wrangler.toml` id → `d1 migrations apply --remote` → **`wrangler deploy`
(cron `7 * * * *` goes live here)** → `secret put AUTH_TOKEN/SPOTIFY_CLIENT_ID/TOKEN_ENCRYPT_KEY`
(`SPOTIFY_USER_ID` defaults `"default"`) → write `WORKER_*` back to .env → optional gh secrets →
`seed_d1.py` (retry loop, pushes local tokens + history to D1).
